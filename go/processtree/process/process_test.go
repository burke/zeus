...
func TestStartZeusProcess(t *testing.T) {
	...
	if have, want := proc.Identifier(), "boot"; have != want {
...
func TestBootNode(t *testing.T) {
	...
	if have, want := child.Identifier(), "foo"; have != want {
		t.Errorf("expected identifier %q, got %q", want, have)
	}
...
func TestBootCommand(t *testing.T) {
	...
	proc, err := startSuccessfully(
		fmt.Sprintf("def boot; end; def foo; exit(%s); end", exitWith),
	)
	if err != nil {
		t.Fatal(err)
	}
	...
	argR, argW, err := os.Pipe()
	if err != nil {
		t.Fatal(err)
	}
	argW.Close()

	clientFile, err := ioutil.TempFile("", "zeus-test-track")
	if err != nil {
		t.Fatal(err)
	}

	req := process.NewCommandRequest("foo", process.CommandClient{
		Pid:      os.Getpid(),
		ArgCount: 0,
		ArgFD:    int(argR.Fd()),
		File:     clientFile,
	})
	proc.Boot(req.BootRequest)
	cmd, err := req.Await()
	if err != nil {
		t.Fatal(err)
	}
	...
	case pid := <-cmd.Ready():
		if pid < 1 {
			t.Errorf("expected command pid to be a positive integer, got %d", pid)
		}
	case err := <-cmd.Errors():
		t.Fatal(err)
	}
	...
	select {
	case exitStatus := <-cmd.Wait():
		if exitStatus != exitWith {
			t.Errorf("expected exit status %s, got %q", exitWith, exitStatus)
		}
	case err := <-cmd.Errors():
		t.Fatal(err)
	}
